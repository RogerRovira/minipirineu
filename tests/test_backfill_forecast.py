...
def test_run_backfill_upserts_rows_and_is_idempotent(tmp_path):
    ...
    # 8 buckets × 2 columns (arome_25 + arome_hd), each with its OPG variant
    # because Z1 is above Baqueira's saturation elevation (S1.3)
    assert n == 32
    ...
    assert conn.execute("SELECT COUNT(*) FROM verification_values").fetchone()[0] == 32
...
def test_run_backfill_writes_forecast_columns_the_verifier_reads(tmp_path):
    ...
    assert variables == {"fx.snowfall_cm.arome_25", "fx.snowfall_cm.arome_hd",
                         "fx.snowfall_cm.arome_25_opg", "fx.snowfall_cm.arome_hd_opg"}
